...
#TODO: worker y terminal deberian ser globales para evitar el paso de parámetros innecesarios
...
class Main:
    ...
    def setup_options(self, worker: Worker, terminal: Terminal):
        """
        ...
        while worker.OPTION_TMP_FILES != 0 and worker.OPTION_TMP_FILES != 1:        
            ...
            worker.OPTION_TMP_FILES = self.answer_from_string_to_binary(input().lower())

        while worker.OPTION_TMP_RECORDS != 0 and worker.OPTION_TMP_RECORDS != 1:
            terminal.writeln("Voleu conservar els registres? (s/n) ")
            worker.OPTION_TMP_RECORDS = self.answer_from_string_to_binary(input().lower())
        ...
        while worker.OPTION_DUPLICATED_ANSWERS != 0 and worker.OPTION_DUPLICATED_ANSWERS != 1:
            ...
            worker.OPTION_DUPLICATED_ANSWERS = self.answer_from_string_to_binary(input().lower())

        while worker.OPTION_REPORTS != 0 and worker.OPTION_REPORTS != 1:
            terminal.writeln("Desitja generar els informes? (s/n) ")
            worker.OPTION_REPORTS = self.answer_from_string_to_binary(input().lower())
    ...
    def offer_navigation_menu_for_troublesome_source_files(self, source_file):
        """
        ...
        if option == "1":
            ...
            input()
            if worker.LOG_LEVEL > 0: terminal.write("Reintentant... ")
            ...
            terminal.untab()
            self.offer_navigation_menu_for_troublesome_source_files(source_file)
